            ...
            <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto md:p-4">
                ...
                <div className="hidden w-full md:block md:w-auto" id="navbar-solid-bg">
                    <ul className="flex flex-col font-medium mt-4 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-transparent dark:bg-gray-800 md:dark:bg-transparent dark:border-gray-700">
                        {
                            links.map((link: Link) => <li key={`${link.id}`}><Link href={`#${link.id}`} className=" underline underline-offset-4 md:no-underline hover:underline hover:underline-offset-4"> {link.title} </Link></li>)
                        }
                    </ul>
                </div>
            </div>
